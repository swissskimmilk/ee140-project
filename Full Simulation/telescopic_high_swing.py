...
def vov_from_gm_id(gm_id, k=2.0):
    """
    Approximate Vov from gm/ID using long-channel relation gm/ID ≈ 2/Vov.
    Vov ≈ k / gm_id, k≈2.
    This is *approximate* and MUST be checked in Spectre afterwards.
    """
    return k / gm_id
...
def design_telescopic(ft_fu_ratio=5,
                      ...
                      sat_margin_p=0.05):
    """
    Telescopic stage-1 designer with high-swing-style PMOS load.

    Additional safety vs original version:
    - Enforces approximate saturation headroom: VDS (or VSD for PMOS)
      must exceed Vov + sat_margin for the devices we are optimizing.
    - Performs post-selection headroom checks and prints warnings if
      anything looks marginal in this gm/ID model.
    """
    ...
    for i, vout in enumerate(Vout_range):
        ...
        VZ = vout + headroom_p_bot
        if VZ >= VDD:
            continue

        VSD_bot_p = headroom_p_bot
        VSD_top_p = VDD - VZ

        # High-swing condition: top PMOS gate tied to bottom drain (ref side),
        # which we treat as at Vout for DC sizing:
        VSG_top_req = VDD - vout
        ...
        for gm_ID_bot_p in gm_ID_bot_p_grid:
            # Approx Vov check for bottom PMOS vs allocated headroom
            vov_bot_est = vov_from_gm_id(gm_ID_bot_p)
            if VSD_bot_p < vov_bot_est + sat_margin_p:
                # This gm/ID would require more VSD than we have; skip
                continue
            ...
            for idx_lp, Lp in enumerate(L_p_range):
                ...
                # Top PMOS (P_top): find gm/ID such that VSG_top ≈ VSG_top_req
                best_gmID_top = None
                best_vsg_err  = None

                for gmID_t in gm_ID_top_p_grid:
                    # Approx Vov check for top PMOS
                ...
                # Require that we found a usable gm/ID for the top PMOS
                if best_vsg_err is None or best_vsg_err > 0.03:
                    continue

                gm_ID_top_p_eff = best_gmID_top

                # Top PMOS gain and fT
                ...
                # bottom ro boosted by cascode: (1 + gm_top * ro_top) ≈ (1 + Av_top_p)
        ...
        for j, L_in in enumerate(L_in_range):
            ...
            VDS_in  = Vx - tail_headroom  # input device VDS
            if VDS_in <= 0:
                best_n_gm_id[i, j] = np.nan
                best_Av_norm[i, j] = np.nan
                continue

            # Approx Vov range for input NMOS vs VDS allocation
            vov_in_range_est = vov_from_gm_id(gm_ID_range_in)

            # Input NMOS (2V) gds/ID and fT vs gm/ID
            ...
            for gm_ID_casc_n in gm_ID_casc_n_grid:
                ...
                # Enforce VBN <= VDD (with a little margin)
                if Vbias_N_casc > VDD:
                    continue

                # Only keep gm/ID points where input NMOS meets ft AND saturation
                K_ft  = (n_ft_in > ft)
    ...
    if np.all(np.isnan(best_Av_norm)):
        print("No valid telescopic design found for given constraints/margins.")
        return None
    ...
    tail_vds_est = vov_from_gm_id(tail_gm_id)  # ~Vov

    if tail_vds_est > tail_headroom:
        print("Tail headroom likely insufficient (Vdsat estimate > allocated).")
    ...
    Vcm_in = tail_headroom + vgs_in

    # Bottom PMOS bias (VBP0)
    vsg_bot = look_up_vgs_vs_gm_id(
        pch, opt_gm_ID_bot_p, vds=VSD_bot_po, l=opt_p_l
    )
    Vbias_P_bot = VZ_opt - vsg_bot

    # Top PMOS gate tied to drain of bottom PMOS on reference side (≈ Vout)
    ...
    #   PRINT RESULTS
    # =====================================================================
    print("\n=== TELESCOPIC HIGH-SWING-LOAD DESIGN RESULTS ===")
    print(f"fu target:             {fu/1e6:.2f} MHz  (ft req: {ft/1e6:.2f} MHz)")
    ...
    print(f"Bottom PMOS gm/ID:     {opt_gm_ID_bot_p:.2f} V⁻¹")
    print(f"Top PMOS gm/ID:        {opt_gm_ID_top_p:.2f} V⁻¹")
    print(f"NMOS cascode gm/ID:    {opt_gm_ID_casc_n:.2f} V⁻¹")
    ...
    print("")
    print(f"Input NMOS (2V):  Wn = {wn:.2f} um,  VDS = {VDS_in_o:.3f} V, Vov≈{vov_in_est:.3f} V")
    print(f"Bottom PMOS:      Wp_bot = {wp_bot:.2f} um, VSD = {VSD_bot_po:.3f} V, Vov≈{vov_p_bot_est:.3f} V")
    print(f"Top PMOS:         Wp_top = {wp_top:.2f} um,  VSD = {VSD_top_po:.3f} V, Vov≈{vov_p_top_est:.3f} V")
    print(f"Tail NMOS:        W_tail = {W_tail:.2f} um, ID_tail = {tail_Id*1e6:.2f} uA, VDS≈{tail_headroom:.3f} V, Vov≈{vov_tail_est:.3f} V")
    print("")
    print(f"Input CM Vcm_in:        {Vcm_in:.3f} V")
    print(f"NMOS cascode bias VBN:  {Vbias_N_casc:.3f} V")
    print(f"PMOS bottom bias VBP0:  {Vbias_P_bot:.3f} V")
    print(f"PMOS top bias   VBP1:   {Vbias_P_top:.3f} V  (gate tied to bottom drain, ref side)")
    print("")
    if violations:
