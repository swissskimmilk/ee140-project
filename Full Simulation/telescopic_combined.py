...
def design_telescopic(ft_fu_ratio=5,
                      ...
                      mode="high_swing"):
    """
    Telescopic stage-1 designer with PMOS active load.
    ...
    for i, vout in enumerate(Vout_range):
        ...
        for gm_ID_bot_p in gm_ID_bot_p_grid:
            ...
            for idx_lp, Lp in enumerate(L_p_range):
                ...
                # bottom ro boosted by cascode-like action:
                # (1 + gm_top * ro_top) ≈ (1 + Av_top_p)
    ...
    # =====================================================================
    # f_u = gm1 / (2 * pi * Cc)
    # After sizing, we use the actual gm_M1 to get the actual fu
    ...
    return {
        ...
        "VDD_spec":       VDD_spec,      # Actual supply voltage used
        "VDD_eff":        VDD_eff,       # Effective supply after margin
    }


if __name__ == "__main__":
    res_hs  = design_telescopic(mode="high_swing")
    res_std = design_telescopic(mode="standard")
